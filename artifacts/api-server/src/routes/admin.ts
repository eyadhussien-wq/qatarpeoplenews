...
if (process.env.NODE_ENV === "production" && (!ADMIN_PASSWORD || !SESSION_SECRET)) {
  // Configuration is intentionally validated at request time as well so the app can boot
  // in deployment environments where secrets are injected after process startup.
}
...
function createSession() {
  const payload: SessionPayload = { sub: "admin", exp: Math.floor(Date.now() / 1000) + SESSION_TTL_SECONDS };
...
export function requireAdmin(req: Request, res: Response, next: NextFunction) {
  if (!isAdminAuthenticated(req)) return res.status(401).json({ error: "Authentication required" });
  return next();
}

function setSessionCookie(res: Response, value: string) {
  res.setHeader("Set-Cookie", `${SESSION_COOKIE}=${encodeURIComponent(value)}; HttpOnly; SameSite=Lax; Path=/; Max-Age=${SESSION_TTL_SECONDS}${process.env.NODE_ENV === "production" ? "; Secure" : ""}`);
}
function clearSessionCookie(res: Response) {
  res.setHeader("Set-Cookie", `${SESSION_COOKIE}=; HttpOnly; SameSite=Lax; Path=/; Max-Age=0${process.env.NODE_ENV === "production" ? "; Secure" : ""}`);
}
...
router.post("/admin/login", (req, res) => {
  ...
  if (!allowLoginAttempt(req)) return res.status(429).json({ error: "Too many login attempts. Try again later." });
...
function dashboardHtml(email: string) {
  return shell("لوحة التحكم", `<main class="dashboard"><div class="top"><div><div class="brand">لوحة تحكم QPN</div><div class="muted">${email}</div></div><button id="logout" class="logout">تسجيل الخروج</button></div><div class="grid"><a class="tile" href="/api/admin/news-manager" style="text-decoration:none;color:inherit"><span class="badge">جاهز</span><h3>الأخبار</h3><div class="muted">إدارة الأخبار، المسودات، النشر والعاجل.</div></a><a class="tile" href="/api/admin/news-sync-dashboard" style="text-decoration:none;color:inherit"><span class="badge">جاهز</span><h3>مزامنة الأخبار</h3><div class="muted">تشغيل News Engine ومتابعة المصادر الرسمية.</div></a><section class="tile"><span class="badge">محمي</span><h3>التلفزيون</h3><div class="muted">مصادر YouTube الحالية لن تتغير.</div></section><section class="tile"><span class="badge">محمي</span><h3>الراديو</h3><div class="muted">مصادر QMC HLS الحالية لن تتغير.</div></section></div><section class="note"><strong>أساس الإدارة جاهز.</strong><br>يمكن الآن إدارة الأخبار وتشغيل المزامنة من لوحة الويب، مع بقاء أسرار الدخول على متغيرات البيئة وعدم تخزينها في المستودع.</section></main>`, `<script>document.getElementById('logout').onclick=async()=>{await fetch('/api/admin/logout',{method:'POST'});location.reload()};</script>`);
}
