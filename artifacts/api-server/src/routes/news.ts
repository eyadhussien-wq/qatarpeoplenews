...
function validateNewsBody(body: unknown) {
  ...
  if (content.length < 1) return { error: "Content is required" };
  ...
  const isBreaking = input?.isBreaking === true;
  return { value: { title, content, status, categoryId, excerpt, coverImageUrl, videoUrl, isBreaking } };
}
...
router.get("/news", async (req, res, next) => {
  try {
    ...
    const rows = await db.select({
      id: news.id, title: news.title, slug: news.slug, excerpt: news.excerpt, coverImageUrl: news.coverImageUrl,
      videoUrl: news.videoUrl, categoryId: news.categoryId, status: news.status, isBreaking: news.isBreaking,
      views: news.views, publishedAt: news.publishedAt, createdAt: news.createdAt, updatedAt: news.updatedAt,
      categoryName: newsCategories.name, categorySlug: newsCategories.slug,
    }).from(news).leftJoin(newsCategories, eq(news.categoryId, newsCategories.id))
      .where(and(eq(news.status, "published"), search ? ilike(news.title, `%${search}%`) : undefined))
      .orderBy(desc(news.isBreaking), desc(news.publishedAt), desc(news.createdAt)).limit(limit);
...
router.post("/admin/news", requireAdmin, async (req, res, next) => {
  try {
    ...
    const { value } = parsed;
    const now = new Date();
    const slugBase = slugify(value.title);
    const [created] = await db.insert(news).values({
      title: value.title, slug: `${slugBase}-${Date.now()}`, content: value.content, excerpt: value.excerpt,
      coverImageUrl: value.coverImageUrl, videoUrl: value.videoUrl, categoryId: value.categoryId,
      status: value.status, isBreaking: value.isBreaking, publishedAt: value.status === "published" ? now : null,
      updatedAt: now,
    }).returning();
...
router.get("/admin/news/categories", requireAdmin, async (_req, res, next) => {
  try { return res.json({ data: await db.select().from(newsCategories).orderBy(newsCategories.name) }); }
  catch (error) { return next(error); }
});
...
router.get("/news/:id", async (req, res, next) => {
  try {
    const [row] = await db.select({ news, category: newsCategories }).from(news).leftJoin(newsCategories, eq(news.categoryId, newsCategories.id)).where(eq(news.id, req.params.id)).limit(1);
    if (!row || row.news.status !== "published") return res.status(404).json({ error: "News not found" });
    return res.json({ data: { ...row.news, category: row.category } });
  } catch (error) { return next(error); }
});
...
router.patch("/admin/news/:id", requireAdmin, async (req, res, next) => {
  try {
    ...
    const [updated] = await db.update(news).set({
      title: value.title, content: value.content, excerpt: value.excerpt, coverImageUrl: value.coverImageUrl,
      videoUrl: value.videoUrl, categoryId: value.categoryId, status: value.status, isBreaking: value.isBreaking,
      publishedAt: value.status === "published" ? now : null, updatedAt: now,
    }).where(eq(news.id, req.params.id)).returning();
